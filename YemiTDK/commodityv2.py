...
class MitsuiCommodityFeatureEngineer:
    """
    Feature engineering specifically designed for MITSUI Commodity Prediction Challenge
    Based on actual dataset structure with LME, JPX, US_Stock, and FX data
    """

    def __init__(self, lookback_windows=[3, 5, 10, 20], volatility_windows=[5, 10, 20]):
        self.lookback_windows = lookback_windows
        self.volatility_windows = volatility_windows
        self.scalers = {}

        # Define column groups based on your actual data structure
        self.lme_columns = [
            "LME_AH_Close",
            "LME_CA_Close",
            "LME_PB_Close",
        ...
        # JPX futures columns (different data types)
        self.jpx_price_cols = [
        ...
        # US Stock columns (will be identified dynamically)
        self.fx_columns = [
            "FX_AUDJPY",
            "FX_AUDUSD",
            "FX_CADJPY",
    ...
    def create_cross_asset_features(self, df):
        ...
        if self.lme_columns and any(col in df.columns for col in self.lme_columns):
            lme_data = df[[col for col in self.lme_columns if col in df.columns]]
    ...
    def engineer_all_features(
        ...
        # Target-based features (only for training data)
...
if __name__ == "__main__":
    ...
    # Load the actual competition data with correct paths
    base_path = r"C:\Users\ayemi\OneDrive\Documents\Commodity_Prediction\mitsui-commodity-prediction-challenge"

    trains_csv = (
        f"{base_path}\\train.csv"  # THIS WAS THE BUG - was loading target_pairs.csv
    )
    tests_csv = f"{base_path}\\test.csv"
    train_labels_csv = f"{base_path}\\train_labels.csv"
    target_pairs_csv = f"{base_path}\\target_pairs.csv"

    results, feature_groups = process_mitsui_data(
        trains_csv, tests_csv, train_labels_csv, target_pairs_csv
    )
    print("Data processing complete.")
    print(f"Train features shape: {results['train'].shape}")
    print(f"Test features shape: {results['test'].shape}")
